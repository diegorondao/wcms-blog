...
if ( isset($_POST['media_edit_form']) ) {
	...
	$fileName = '';
	$upload = doUpload(BLOG_UPLOAD_DIR, 'upload_file_name', $fileName, true,
					   ['.jpg', '.png', '.jpeg', '.docx', '.doc', '.pdf'] );

	if ( $upload === true ) {
		$upload_data = ['upload_name' => _post('upload_name'), 'upload_file_name' => $fileName];
		if ( $media_id ) {
			$old_data = blog_db_select('uploads', ['*'], ['ID' => $media_id])[0];
			@unlink( BLOG_UPLOAD_DIR . '/' . $old_data->upload_file_name);

			if ( blog_db_update('uploads', $upload_data, ['ID' => $media_id] ) ) { 
			...
			if ( blog_db_insert('uploads', $upload_data) ) {
