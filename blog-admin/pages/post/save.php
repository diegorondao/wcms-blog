...
if ( isset($_POST['post_edit_form']) ) {
	...
	if ( $post_id ) {
		if ( blog_db_update( 'posts' , $data , ['ID' => $post_id ] ) ) {
			blog_assign_category($post_id, $_POST['post_categories']);
		...
		$created_post = blog_db_insert('posts', $data);
		if ( $created_post ) {
			blog_assign_category($created_post, $_POST['post_categories']);
