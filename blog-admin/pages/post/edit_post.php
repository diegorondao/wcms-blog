...
if ( $post_id ) {
	...
	$post = blog_db_select('posts', ['*'], ['ID' => $post_id])[0];
	...
	$post_categories = blog_db_select('post_categories', ['*'], ['posts_ID' => $post_id]);
	...
	<div class="form-group">
		...
		<div class="col-md-8">
			<select name="post_author" class="form-control">
				<?php 
					$users = blog_fetch_all_users();
					foreach( $users as $user) :
				?>
				<option <?php 
					if ( ( ! $post_id && blog_get_current_user_ID() == $user->ID) || ($post_author == $user->ID) ) 
						echo 'selected';
				?>
		...
		<div class="form-group">
			...
			<div class="col-md-8">
				<select multiple name="post_categories[]" class="form-control">
					<?php 
					$categories = blog_fetch_all_cats();
					if ( $categories) : ?>
		...
		<div class="form-group">
			...
			<div class="col-sm-8">
			  	<select name="post_image" class="form-control">
			  		...
			  		<?php
			  			$uploads = blog_fetch_all_uploads();
