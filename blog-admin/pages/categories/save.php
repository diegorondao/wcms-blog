...
if ( isset($_POST['categories_edit_form']) ) {
	...
	if ( $cat_id ) {
		if ( blog_db_update('categories', $data, ['ID' => $cat_id]) ) {
			redirect('edit_category', ['successMsg' => 'Categoria Salva com sucesso', 'cat_id' => $cat_id]);
		}else {
			redirect('edit_category', ['errorMsg' => 'Erro ao atualizar categoria'] );
		}
	} else {
		if ( blog_db_insert('categories', $data) ) {
