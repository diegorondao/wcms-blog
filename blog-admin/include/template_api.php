...
function get_header( $suffix = '' ) {
	...
	include( BLOG_SITE_DIR . '/header' . $_suffix . '.php');
}

function get_footer( $suffix = '' ) {
	$_suffix = empty($suffix) ?  '' : '-' . $suffix;

	include( BLOG_SITE_DIR . '/footer' . $_suffix . '.php');
} 

function get_sidebar( $suffix = '' ) {
	$_suffix = empty($suffix) ?  '' : '-' . $suffix;

	include( BLOG_SITE_DIR . '/sidebar' . $_suffix . '.php');	
} 

function blog_get_menu_pages( $nPages = -1 ) {
	$nPages = ($nPages == -1) ? [] : [0,$nPages];
	return blog_db_select('posts', ['*'], ['post_type' => 'page'], $nPages, ['post_order', 'ASC'] );
}
...
function get_permalink($action, Array $params = null) {
	...
	return BLOG_BASE_URL . '/index.php?view=' . $action . $sParams;
}
