...
function blog_fetch_all_posts( $post_type ) {
	return blog_db_select('posts', ['*'], ['post_type' => $post_type], [], ['post_date', 'DESC']);
}

function blog_assign_category( $post_id, $categories ) {
	if ( ! $categories ) return false;

	if ( blog_db_delete('post_categories', ['posts_ID' => $post_id] ) ) {
		foreach( $categories as $cat ) {
			blog_db_insert('post_categories', ['posts_ID' => $post_id, 'categories_ID' => $cat]);
		}
...
function get_post() {
	...
	if ( $post ) {
		$post = blog_db_select('posts', ['*'], ['ID' => $post] );
		if ( $post ) {
			return $post[0];
		} else {
			return false;
...
function get_posts($postsPerPage = 5, $post_type = 'post', $cat_id = null) {
	...
	$totalPosts = 0;
	if ( is_null($cat_id) ) {
		$totalPosts = blog_db_select('posts', ['count(ID) as total'], ['post_type' => $post_type] )[0]->total;
		$posts = blog_db_select('posts', ['*'],['post_type' => $post_type],[$offset, $postsPerPage], ['post_date', 'DESC']);
	} else {
		$conn = getConnection();
...
function get_post_author( $post ) {
	$author = blog_db_select('users', ['*'], ['ID' => $post->post_author]  );
